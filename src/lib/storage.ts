...
function fixUnlockedEpochs(saved: EpochId[], level: number, currentEpochId: EpochId, prestigeLevel: number): EpochId[] {
  const result = new Set<EpochId>(saved);
  result.add(currentEpochId);
  for (const epoch of EPOCHS) {
    const requiredPrestige = epoch.requiredPrestige ?? 0;
    if (epoch.unlockLevel <= level && requiredPrestige <= prestigeLevel) {
...
function hydrateFromDb(data: Record<string, unknown>): GameState {
  ...
  return {
    ...
    unlockedEpochs: fixUnlockedEpochs(
      ...
      (data.prestige_level as number) || 0,
    ),
