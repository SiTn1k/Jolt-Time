...
export function GachaModal({
  ...
  const handleRoll = useCallback(async () => {
    ...
    if (!telegramId) {
      setErrorMessage('Помилка: скрині доступні лише в Telegram Mini App');
      setPhase('error');
      return;
    }
    ...
    if (!result.ok) {
      setErrorMessage(result.error || 'Не вдалося відкрити скриню. Спробуйте пізніше.');
      setPhase('error');
      return;
    }

    if (!result.rewards || result.rewards.length === 0) {
      setErrorMessage('Сервер не повернув нагород. Спробуйте ще раз.');
      setPhase('error');
      return;
    }
