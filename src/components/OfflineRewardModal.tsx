...
import { initAdsgram, waitForAdsgramSDK, showRewardAd } from '../services/adsgram';
...
export function OfflineRewardModal({
  ...
  const handleWatchAd = useCallback(async () => {
    // Try to get SDK, wait if not ready
    let sad = initAdsgram();
    if (!sad) {
      console.log('[OfflineRewardModal] SDK not ready, waiting...');
      sad = await waitForAdsgramSDK(3000);
    }
    ...
    if (!telegramId) {
      setAdError('Помилка авторизації. Відкрийте гру в Telegram.');
      return;
    }
