 ...
 * Snapshot Validator
 *
 * Validates snapshot data according to production rules.
 */
...
export class SnapshotValidator {
  ...
  /**
   * Validates a system health status.
   */
  ...
  public static validate(data: {
    ...
    if (data.healthStatus !== undefined && !this.isValidHealthStatus(data.healthStatus)) {
      errors.push('Health status must be one of: healthy, degraded, unhealthy');
    }
