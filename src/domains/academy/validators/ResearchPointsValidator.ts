/**
 * Research Points Validator
 *
 * Validates research points data according to game rules.
 */

import { ResearchPoints, MAX_RESEARCH_POINTS } from '../value-objects/ResearchPoints';

/**
 * Result of research points validation.
 */
export interface ResearchPointsValidationResult {
  isValid: boolean;
  errors: string[];
}

/**
 * Validator for research points data.
 */
export class ResearchPointsValidator {
  /**
  ...
  public static isValid(amount: number | null | undefined): boolean {
    ...
    return ResearchPoints.isValid(amount);
  }
  ...
  /**
   * Validates complete research points operation.
  ...
  public static validate(data: {
    ...
    if (data.amount !== undefined) {
      ...
      } else if (data.amount > MAX_RESEARCH_POINTS) {
        errors.push(`Amount exceeds maximum allowed value: ${MAX_RESEARCH_POINTS}`);
      }
    }
    ...
    if (
      ...
      errors.push(`Insufficient research points: current=${data.currentBalance}, required=${data.requiredAmount}`);
    }
  ...
  /**
   * Validates research points data and throws if invalid.
   * @param data The data to validate
  ...
  public static validateOrThrow(data: {
    ...
    if (!result.isValid) {
      throw new Error(`Research points validation failed: ${result.errors.join('; ')}`);
    }
  }
}