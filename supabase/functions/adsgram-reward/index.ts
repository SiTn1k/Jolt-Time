...
async function handleGetCallback(params: URLSearchParams) {
  // AdsGram may send userId (camelCase) or userid (lowercase)
 ...
 */
async function handlePostCallback(body: { userid?: string; userId?: string; ad_id?: string; reward_type?: string }) {
  // Support both userid and userId
  const { userid, userId, ad_id, reward_type } = body;
  ...
  const telegramId = parseInt(userIdValue, 10);
  if (isNaN(telegramId) || telegramId <= 0) {
    return jsonResponse({ error: "Invalid userid" }, 400);
  }
